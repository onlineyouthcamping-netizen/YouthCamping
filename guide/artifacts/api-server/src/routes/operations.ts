...
operationsRouter.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!type || type === "timing") {
        const cond = [];
        if (parsedAssignmentId) cond.push(eq(guideTripUpdatesTable.assignmentId, parsedAssignmentId));
        if (parsedGuideId) cond.push(eq(guideTripUpdatesTable.guideId, parsedGuideId));
        const updates = await db.select().from(guideTripUpdatesTable).where(and(...cond)).orderBy(desc(guideTripUpdatesTable.createdAt));
      ...
      if (!type || type === "checkin") {
        const cond = [];
        if (parsedAssignmentId) cond.push(eq(guideCheckinPointsTable.assignmentId, parsedAssignmentId));
        if (parsedGuideId) cond.push(eq(guideCheckinPointsTable.guideId, parsedGuideId));
        const checkins = await db.select().from(guideCheckinPointsTable).where(and(...cond)).orderBy(desc(guideCheckinPointsTable.createdAt));
      ...
      if (!type || type === "hotel") {
        const cond = [];
        if (parsedAssignmentId) cond.push(eq(guideHotelUpdatesTable.assignmentId, parsedAssignmentId));
        if (parsedGuideId) cond.push(eq(guideHotelUpdatesTable.guideId, parsedGuideId));
        if (status) cond.push(eq(guideHotelUpdatesTable.status, status as string));
        const hotels = await db.select().from(guideHotelUpdatesTable).where(and(...cond)).orderBy(desc(guideHotelUpdatesTable.createdAt));
      ...
      if (!type || type === "food") {
        const cond = [];
        if (parsedAssignmentId) cond.push(eq(guideFoodUpdatesTable.assignmentId, parsedAssignmentId));
        if (parsedGuideId) cond.push(eq(guideFoodUpdatesTable.guideId, parsedGuideId));
        const foods = await db.select().from(guideFoodUpdatesTable).where(and(...cond)).orderBy(desc(guideFoodUpdatesTable.createdAt));
      ...
      if (!type || type === "group_photo") {
        const cond = [];
        if (parsedAssignmentId) cond.push(eq(guideGroupPhotosTable.assignmentId, parsedAssignmentId));
        if (parsedGuideId) cond.push(eq(guideGroupPhotosTable.guideId, parsedGuideId));
        const groupPhotos = await db.select().from(guideGroupPhotosTable).where(and(...cond)).orderBy(desc(guideGroupPhotosTable.createdAt));
      ...
      if (!type || type === "movement") {
        const cond = [];
        if (parsedAssignmentId) cond.push(eq(guideMovementUpdatesTable.assignmentId, parsedAssignmentId));
        if (parsedGuideId) cond.push(eq(guideMovementUpdatesTable.guideId, parsedGuideId));
        const movements = await db.select().from(guideMovementUpdatesTable).where(and(...cond)).orderBy(desc(guideMovementUpdatesTable.createdAt));
    ...
    try {
      ...
      for (const a of activeAssignments) {
        ...
        const hotelUpdates = await db
          .select()
          .from(guideHotelUpdatesTable)
          .where(eq(guideHotelUpdatesTable.assignmentId, a.id));
        ...
        const foodUpdates = await db
          .select()
          .from(guideFoodUpdatesTable)
          .where(eq(guideFoodUpdatesTable.assignmentId, a.id));
        ...
        const groupPhotos = await db
          .select()
          .from(guideGroupPhotosTable)
          .where(eq(guideGroupPhotosTable.assignmentId, a.id));
        ...
        if (currentHour >= 8 && currentHour <= 20) {
          const movementUpdates = await db
            .select()
            .from(guideMovementUpdatesTable)
            .where(eq(guideMovementUpdatesTable.assignmentId, a.id))
            .orderBy(desc(guideMovementUpdatesTable.createdAt));
        ...
        if (a.mainBackendTripId) {
          const bookings = await fetchBookingsForTrip(a.mainBackendTripId).catch(() => []);
          let jainPreferenceCount = 0;
          let nonJainPreferenceCount = 0;
          ...
          const [latestFoodUpdate] = await db
            .select()
            .from(guideFoodUpdatesTable)
            .where(eq(guideFoodUpdatesTable.assignmentId, a.id))
            .orderBy(desc(guideFoodUpdatesTable.createdAt))
            .limit(1);
