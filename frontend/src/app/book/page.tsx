  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        console.warn("Could not fetch global settings in booking form.", err);
      }
    };
    loadSettings();
  }, []);
    ...
    <div className="bg-[#FAFAFA] min-h-screen text-slate-900 pb-36 lg:pb-36">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* Sticky Bottom Pricing Bar (Mobile & Desktop) */}
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] p-3.5 z-50">
            <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              {/* Left Side: Price and Ask button */}
              <div className="flex items-center justify-between sm:justify-start sm:gap-6 w-full sm:w-auto px-1">
                <div>
                  <p className="text-lg md:text-xl font-black text-slate-900 tracking-tight leading-none">₹{pricing.finalTotal.toLocaleString()}</p>
                  <p className="text-[9px] md:text-[10px] text-slate-500 font-semibold mt-0.5">₹{(pricing.finalTotal - pricing.gstAmount).toLocaleString()} + ₹{pricing.gstAmount.toLocaleString()} GST</p>
                </div>
                <a
              ...
              <div className="w-full sm:w-64 shrink-0">
                {currentStep < 4 ? (
                  <button
                    onClick={handleNext}
                    type="button"
                    className="w-full bg-gradient-to-r from-[#FF5B00] to-[#FF8A00] hover:from-[#E65200] hover:to-[#FF7700] text-white rounded-full py-4 sm:py-3.5 px-6 font-extrabold text-xs text-center transition-all shadow-lg shadow-[#FF5B00]/20 uppercase tracking-wider"
                  >
                    Book My Spot
                  </button>
                ) : (
                  <button
                    ...
                    disabled={loading}
                    type="button"
                    className="w-full bg-gradient-to-r from-[#FF5B00] to-[#FF8A00] hover:from-[#E65200] hover:to-[#FF7700] text-white rounded-full py-4 sm:py-3.5 px-6 font-extrabold text-xs text-center transition-all shadow-lg shadow-[#FF5B00]/20 disabled:opacity-50 flex items-center justify-center gap-2 uppercase tracking-wider"
                  >
