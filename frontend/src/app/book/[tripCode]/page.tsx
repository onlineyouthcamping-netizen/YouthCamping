      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-900 to-slate-900 p-6">
        <div className="text-center max-w-md">
          ...
          <p className="text-emerald-200 mb-4">
            Your booking for <strong>{tripName}</strong> has been received.
          </p>
          <div className="bg-white/10 backdrop-blur rounded-xl p-4 mb-6">
            <p className="text-xs text-white/50 capitalize tracking-widest mb-1">
              Your Booking ID
            </p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 py-8 px-4">
      <div className="max-w-lg mx-auto">
        <div className="text-center mb-8">
          <div className="inline-block bg-primary-orange/20 text-[#D4541A] px-4 py-1 rounded-full text-xs font-bold capitalize tracking-widest mb-3">
            {tripCode}
          </div>
          ...
          <p className="text-gray-400 text-sm mt-2">
            Fill in your details to book this trip
          </p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-6 space-y-4">
            <h3 className="text-xs font-bold text-white/40 capitalize tracking-widest flex items-center gap-2">
            ...
            <input
              ...
              className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/30 text-sm focus:ring-2 focus:ring-[#D4541A] outline-none"
            />
            <div className="grid grid-cols-3 gap-3">
              <input
                ...
                className="col-span-2 bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/30 text-sm focus:ring-2 focus:ring-[#D4541A] outline-none"
              />
              <input
                type="number"
                min={1}
            ...
            <select
              ...
              className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white text-sm focus:ring-2 focus:ring-[#D4541A] outline-none"
            >
          ...
          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-6 space-y-4">
            <h3 className="text-xs font-bold text-white/40 capitalize tracking-widest flex items-center gap-2">
              <Train className="w-3.5 h-3.5" /> Travel Details
            </h3>
            <select
              value={form.trainClass}
              onChange={(e) => setForm({ ...form, trainClass: e.target.value })}
              className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none"
            >
              <option value="Sleeper" className="bg-slate-800">
                Sleeper
              </option>
              <option value="3AC" className="bg-slate-800">
                3AC
              </option>
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, ticketStatus: e.target.value })
              }
              className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none"
            >
              <option value="Not Booked" className="bg-slate-800">
                Not Booked
              </option>
              <option value="Confirmed" className="bg-slate-800">
                Confirmed
              </option>
              <option value="Waiting" className="bg-slate-800">
                Waiting
              </option>
            </select>
            <input
              placeholder="Room Type (e.g. Triple Sharing)"
              value={form.roomType}
              onChange={(e) => setForm({ ...form, roomType: e.target.value })}
              className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/30 text-sm outline-none"
            />
          </div>

          <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-6 space-y-4">
            <h3 className="text-xs font-bold text-white/40 capitalize tracking-widest">
              Notes
            </h3>
            <textarea
              ...
              className="w-full bg-white/10 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/30 text-sm min-h-[80px] outline-none"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full bg-gradient-to-r from-orange-600 to-orange-550 hover:from-orange-500 hover:to-orange-450 text-white font-bold capitalize tracking-widest py-4 rounded-2xl text-sm transition-all flex items-center justify-center gap-2 shadow-lg shadow-orange-500/25"
          >
            {submitting ? (
