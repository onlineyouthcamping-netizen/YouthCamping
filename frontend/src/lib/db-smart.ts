...
export async function getQuotationSmart(idOrSlug: string, isAdmin: boolean = false, token: string = ''): Promise<Quotation | null> {
    console.log(`[db-smart] Fetching: "${idOrSlug}" (isAdmin: ${isAdmin}, token: ${token})`);
