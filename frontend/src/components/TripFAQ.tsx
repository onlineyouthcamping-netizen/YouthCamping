  ...
  return (
    <section className="relative">
      <div className="bg-white border border-zinc-100 rounded-[40px] p-10 md:p-14 shadow-sm">
        <h2 className="text-2xl font-bold text-navy mb-10">FAQ</h2>
        <div className="space-y-4">
          {displayFaqs.map((faq, i) => (
            <div key={i} className="border border-zinc-100 rounded-[25px] overflow-hidden bg-white shadow-sm">
              <button
                onClick={() => setOpenIndex(openIndex === i ? null : i)}
                className="w-full flex items-center justify-between p-8 text-left hover:bg-zinc-50 transition-all animate-fade-in"
              >
              ...
              <div className={cn(
                ...
                <div className="overflow-hidden">
                  <p className="p-8 pt-0 text-zinc-500 font-medium leading-relaxed border-t border-zinc-50">
                    {faq.answer}
                  </p>
                </div>
              </div>
